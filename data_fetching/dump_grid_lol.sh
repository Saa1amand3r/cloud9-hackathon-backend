...
PAGE_SIZE=50
RETRIES=5
DELAY_SECONDS=4   # ~15 requests/minute
STATE_DELAY_SECONDS=4
STATE_RETRIES=5
SKIP_TOURNAMENTS=${SKIP_TOURNAMENTS:-0}
SKIP_SERIES=${SKIP_SERIES:-0}
SKIP_STATE=${SKIP_STATE:-0}
TRUNCATE_TOURNAMENTS=${TRUNCATE_TOURNAMENTS:-1}
TRUNCATE_SERIES=${TRUNCATE_SERIES:-1}
TRUNCATE_STATE=${TRUNCATE_STATE:-1}
...
if [[ "$SKIP_TOURNAMENTS" != "1" && "$TRUNCATE_TOURNAMENTS" == "1" ]]; then
  : > "$OUT_TOURNAMENTS"
fi
if [[ "$SKIP_SERIES" != "1" && "$TRUNCATE_SERIES" == "1" ]]; then
  : > "$OUT_SERIES"
fi
if [[ "$SKIP_STATE" != "1" && "$TRUNCATE_STATE" == "1" ]]; then
  : > "$OUT_SERIESSTATE"
fi
...
if isinstance(d,dict) and d.get("errors") and not d.get("data"):
    # Rate limit -> signal retry
    msg=str(d.get("errors")[0].get("message","")) if d.get("errors") else ""
...
if [[ "$SKIP_TOURNAMENTS" == "1" ]]; then
  if [[ ! -s "$OUT_TOURNAMENTS" ]]; then
    echo "SKIP_TOURNAMENTS=1 but $OUT_TOURNAMENTS is missing/empty." >&2
    exit 1
  fi
else
...
if [[ "$SKIP_SERIES" == "1" ]]; then
  if [[ ! -s "$OUT_SERIES" ]]; then
    echo "SKIP_SERIES=1 but $OUT_SERIES is missing/empty." >&2
    exit 1
  fi
else
  if [[ "$tournament_ids" == "[]" ]]; then
    echo "No tournaments found. Check GRID_API_KEY, TITLE_ID, and filters." >&2
    exit 1
  fi
...
if [[ "$SKIP_STATE" == "1" ]]; then
  echo "Skipping seriesState fetch."
else
  echo "=== Fetch seriesState for each series ==="
  GRID_API_KEY="$GRID_API_KEY" STATE_URL="$STATE_URL" STATE_DELAY_SECONDS="$STATE_DELAY_SECONDS" STATE_RETRIES="$STATE_RETRIES" "$PYTHON" - <<'PY'
import json
import os
import pathlib
import subprocess
...
delay = float(os.environ.get("STATE_DELAY_SECONDS", "4"))
retries = int(os.environ.get("STATE_RETRIES", "5"))
...
if out_file.exists():
    for line in out_file.read_text(encoding="utf-8").splitlines():
        try:
            seen.add((json.loads(line) or {}).get("series_id"))
        except Exception:
            pass

def fetch_state(sid: str):
    payload = {
        "query": (
            "query SS($id:ID!){"
            "seriesState(id:$id){"
    ...
    for attempt in range(1, retries + 1):
        ...
        if data.get("errors") and not data.get("data"):
            msg = str((data.get("errors") or [{}])[0].get("message", ""))
            if "rate limit" in msg.lower():
...
for sid in series:
    ...
    count += 1
    if count % 50 == 0:
        print(f"fetched {count} seriesState records...")
    time.sleep(delay)
PY
fi
